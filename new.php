        ...
        if (!isset($response) || $response === false || $httpCode < 200 || $httpCode >= 300) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                ...
                if (!$title) {
                    ...
                    // Build prefill structure instead of saving directly
...
<!doctype html>
<html lang="en" x-data='recipeForm(<?= json_encode($prefill, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'>
<head>
  <meta charset="utf-8">
    ...
    <div class="card-body">
      ...
      <form method="post" onsubmit="return requestPinAndSubmit(this)">
        ...
        <div class="mb-3">
          ...
          <input name="title" class="form-control" required x-model="title">
        </div>

        <div class="mb-3">
          <label class="form-label">Description</label>
          <textarea name="description" class="form-control" x-model="description"></textarea>
        </div>

        <div class="mb-3">
          <label class="form-label">Image URL (optional)</label>
          <input name="image" class="form-control" placeholder="https://example.com/photo.jpg" x-model="image">
        ...
        <div class="mb-3">
          ...
          <input name="tags" class="form-control" placeholder="e.g. italian, pasta, quick, vegetarian" x-model="tags">
...
function recipeForm(initial) {
  initial = initial || {};
  const mapSteps = (arr) => (Array.isArray(arr) && arr.length)
    ? arr.map(s => (typeof s === 'string' ? { text: s } : { text: (s && s.text) ? s.text : '' }))
    : [{ text: '' }];
  const mapIngs = (arr) => (Array.isArray(arr) && arr.length)
    ? arr.map(i => ({ name: i.name || '', quantity: i.quantity || '', unit: i.unit || '' }))
    : [{ name: '', quantity: '', unit: '' }];
  return {
    title: initial.title || '',
    ...
    image: initial.image || '',
    tags: initial.tags || '',
    ingredients: mapIngs(initial.ingredients),
    steps: mapSteps(initial.steps),
